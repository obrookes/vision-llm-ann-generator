...
def process_video(runner, video_path: str, prompt: str, checkpoint: str, out_dir: Path,
                   stem: str, score_thresh, max_frames, no_overlay: bool) -> dict:
    ...
    frame_results = runner.track(video_path, prompt, max_frames=max_frames)
    ...
    if not no_overlay:
        ...
        render(video_path, doc, out_dir / f"{stem}.mp4")
...
def main():
    ...
    runner = Sam3Runner(checkpoint=checkpoint, device=args.device, score_thresh=args.score_thresh)
